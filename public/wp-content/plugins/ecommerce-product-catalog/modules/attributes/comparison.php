...
class ic_attribute_comparison {
	...
	function choose( $i = 0, $exclude = null ) {
		...
		$select	 .= ic_select_product( $label, $selected_product_id, 'compare[' . $i . ']', 'ic_self_submit', 0, $exclude );
		$select	 .= '</form>';
		return $select;
	}
	...
	function settings_page() {
		...
		<table>
			<tr>
				...
				<td>
					<?php
					select_page( 'ic_attributes_compare[url]', __( 'Comparison Disabled', 'ecommerce-product-catalog' ), $settings[ 'url' ] );
					?>
				</td>
			</tr>
		</table>
